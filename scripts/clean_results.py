#!/usr/bin/env python
"""
#**PlantCV workflow cleaning results**
...
def main():
    ...
    res = pd.read_csv("/content/work/outputs/raw_results-single-value-traits.csv")
    
    # Remove extra files if real raw format is not interested output.json
    files = ['/content/work/outputs/raw_results-multi-value-traits.csv',
             '/content/work/outputs/output.json',
             '/content/work/outputs/output.json-multi-value-traits.csv', 
             '/content/work/outputs/output.json-single-value-traits.csv']
    for file in files:
        if(os.path.exists(file) and os.path.isfile(file)): 
          os.remove(file) 

    to_drop=['image','image_height', 'image_width','run_date','plantcv_version','object_in_frame_none','in_bounds_none','ellipse_eccentricity_none',
    ...
    # Save with new name
    path_csv = "/content/drive/MyDrive/Fotos_Scanner/Analyze/outputs/" + output_name
